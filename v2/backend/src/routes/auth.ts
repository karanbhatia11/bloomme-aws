...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        if (siteMode !== 'none') {
            console.log('Site mode active, returning specialized response');
        ...
        console.log('Coming soon disabled, generating token');
    ...
    try {
        ...
        if (siteMode !== 'none' && userRole !== 'admin') {
            ...
            return res.status(200).json({
                user: { id, name, email: userEmail, role: userRole, referral_code, referral_points },
                coming_soon: siteMode === 'coming_soon',
                maintenance: siteMode === 'maintenance',
        ...
        res.json({ token, user: { id, name, email: userEmail, role: userRole, referral_code, referral_points } });
    } catch (err: any) {
